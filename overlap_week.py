...
#     if i < df.shape[0]-15 :
#         for j in range(1,15) :
#             if (df.iloc[i]['일자'] - dt.timedelta(days=15) <= df.iloc[i+j]['일자'] < df.iloc[i]['일자']) & \
...
#             if (df.iloc[i]['일자'] - dt.timedelta(days=15) <= df.iloc[i+j]['일자'] < df.iloc[i]['일자'])  & (df.iloc[i]['ESG'] == df.iloc[i+j]['ESG']) \
...
#     if i < df.shape[0]-15 :
#         for j in range(1,15) :
#             if (df.iloc[i]['일자'] < df.iloc[i+j]['일자'] <= df.iloc[i]['일자'] + dt.timedelta(days=15)) & (df.iloc[i]['ESG'] == df.iloc[i+j]['ESG']) \
#                 & (df.iloc[i]['기업'] == df.iloc[i+j]['기업']) :
...
# df.to_excel('C:/ESG/'+file_name+'_AR_15일중복.xlsx', index=None)
...
file_name = 'DF_최종_2_AR_15일중복_중복5.xlsx'
df = pd.read_excel('C:/ESG/'+file_name, engine='openpyxl')
